...
- **Treat a gate block as the system working.** The git gate stops the git operations you declared off-limits until the session verifies them against your rules and re-issues the command with the `CLAUDE_REVIEWED=1` prefix; the style gate names the file and line of any banned text the moment it lands. The block message always says what happens next. The quickstart's "Verify it worked" list is a good first-day drill for seeing each gate fire once.
