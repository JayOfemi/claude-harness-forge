...
2. **The style gate's rules** (`hooks/house-rules.mjs`). This file is executed code, not data: the style gate imports and runs it after every edit, so editing it (or anything else in `hooks/`) is the same as granting code execution in your sessions. Treat that folder as part of your trusted base and review changes to it the way you would review a script you are about to run. The shipped file carries one demo rule so the machinery provably works. Replace it with your actual bans - the things that, if they appear in shipped text or code, embarrass you. The worked example (`examples/house-rules-example.mjs`) shows a full rule-pack with severities. Whatever you ban in the craft globals prose, wire here too, so the machine enforces what the prose declares.

3. **The git gate's hard lines** (`hooks/hard-lines.txt`). Name the operations an agent must never do on its own, like which repos it may push, whether it deploys, and anything money-touching. These belong in BOTH the craft globals (prose, Tier 1) and the hard-lines file (enforcement; the gate quotes it in every block message), verbatim. The rules interview (say "fill my rules") writes this file for you. The gate's non-git coverage (deploy and publish CLIs, cloud mutations) reads from `hooks/gated-tools.txt`, shipped with generic defaults; trim the tools you do not use and add your own. When you add hooks of your own, one wiring rule is load-bearing. Never attach two input-rewriting (updatedInput) PreToolUse hooks to the same tool, because matching hooks run in parallel and the harness does not define which rewrite wins.
...
7. **Your never-publish list** (`deny-list.txt` at your root, beside `tools/`). Seed it with your name, employer, internal project names, domains, and paths, then run `node tools/deny-sweep.mjs <folder>` before anything in this workspace goes public, ever. An empty list makes the sweep refuse rather than certify, so seeding it is what turns the gate on.
