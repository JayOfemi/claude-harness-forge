...
3. **The git gate's hard lines** (`hooks/git-gate.mjs`). Name the operations an agent must never do on its own, like which repos it may push, whether it deploys, and anything money-touching. These belong in BOTH the craft globals (prose, Tier 1) and the gate message (enforcement), verbatim. When you add hooks of your own, one wiring rule is load-bearing. Never attach two input-rewriting (updatedInput) PreToolUse hooks to the same tool, because matching hooks run in parallel and the harness does not define which rewrite wins.
