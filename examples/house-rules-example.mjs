#!/usr/bin/env node
// Example rule-pack for the style gate. This is ONE operator's worked example - the
// MECHANISM is the RULES export; the gate imports whatever rules file you point it at.
// Replace the patterns and messages with your own house rules, or start from these and
// add or remove entries.
//
// Usage (standalone scan):
//   node house-rules-example.mjs [--json] [--stdin] [files or dirs]
//
// Import in your gate:
//   import { RULES, scanText } from "./house-rules-example.mjs";
...
export const RULES = [
	...
	},
	{
		...
		// docs or a deliberately technical section. A clean mechanical pass never
		// certifies the wording is intuitive; that judgment is the human's (step 4).
		id: "surface-jargon",
		severity: "review",
	...
	{
		// Stacked relative clauses padding a noun: "the tool, the command-line
		// interface, which I have used daily and which is my primary client". Keep
		// the one essential detail or split the sentence.
...
function usage() {
	process.stderr.write("usage: house-rules-example.mjs [--json] [--stdin] [files or dirs]\n");
	process.exit(2);
}
