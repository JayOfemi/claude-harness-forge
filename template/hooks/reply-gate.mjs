#!/usr/bin/env node
// Stop-hook reply gate: mechanically checks the turn's final reply against
// the reply-discipline standard's objective half - the prose-length ceiling
// only, fenced and inline code stripped first. On a violation it blocks the
// stop ONCE (exit 2) with exact findings so the model runs a compression
// pass; the stop_hook_active flag means the revised reply always passes, so
// a loop is impossible. The judgment half of reply discipline stays with the
// model and the owner. Infra errors degrade to silent pass (the justified
// quiet-degrade: a broken detector must not trap every session at stop).
// REPLY_GATE_MAX_WORDS (positive number; Infinity disables) overrides the
// ceiling; REPLY_GATE_DEBUG=1 prints stats to stderr.
...
const findings = [];
if (words > MAX_WORDS) {
	findings.push(`${words} words of prose (ceiling ${MAX_WORDS}). Run a compression pass (STANDARDS/reply-discipline.md).`);
}
if (DEBUG) {
