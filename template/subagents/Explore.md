...
This agent is pinned to the cheap tier by the model-routing switch (`STANDARDS/model-routing.md`). Known limit: on tool-heavy sessions a Haiku-pinned Explore can fail with a prompt-too-long error because the tool-definition surface exceeds its context. If the caller sees that failure, the documented fallback is re-spawning with the model parameter set to sonnet, not removing this pin.
