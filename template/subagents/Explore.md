...
This seat is DYNAMIC by default in the model-routing switch (`STANDARDS/model-routing.md`): it resolves to haiku, the floor tier, which by construction never exceeds the session's own tier. Known limit: on tool-heavy sessions a Haiku Explore can fail with a prompt-too-long error because the tool-definition surface exceeds its context; the documented fallback is re-spawning one tier up, never above the session's tier, not removing the haiku default.
